...
class BaseHbasePerScoreWriterVersion1(BasePerScoreWriter, ABC):
    """
    ...
    def __init__(
        ...
        """
        super(BaseHbasePerScoreWriterVersion1, self).__init__()
    ...
    def _gen_key(self, user: User) -> bytes:
        """
        Gen row key for user
        :param user: user to gen key
        :return: hbase key
        """
        mode: int = user.user_id % 1000
        key: str = f"{mode:03d}_{user.user_id}"
    ...
    def write_score(
        ...
        try:
            ...
            SingletonLogger.get_instance().exception(
                "Exception while writing per result to Hbase"
            )
            return False
        ...
        try:
            with self.connector.get_connection() as connection:
                ...
                with table.batch(batch_size=self.batch_size) as batch:
                    if self.verbose:
                        progress_bar = tqdm(
                            iterable=user_to_result.items(),
                            desc="Writing per score to Hbase..."
                        )
                        for user, posts_scores in progress_bar:
                            key: bytes = self._gen_key(user=user)
                            row_dict: Dict[bytes, bytes] = self._posts_scores_to_row_dict(
                                posts_scores=posts_scores
                            )
                        ...
                        for user, posts_scores in user_to_result.items():
                            key: bytes = self._gen_key(user=user)
                            row_dict: Dict[bytes, bytes] = self._posts_scores_to_row_dict(
                                posts_scores=posts_scores
                            )
            ...
            SingletonLogger.get_instance().exception(
                "Exception while writing per results to Hbase"
            )
            return False


class BaseHbasePerScoreWriterVersion1Config(BasePerScoreWriterConfig, ABC):
    """
    ...
    def __init__(
        ...
        :param verbose: display progress bar
        """
        super(BaseHbasePerScoreWriterVersion1Config, self).__init__()
