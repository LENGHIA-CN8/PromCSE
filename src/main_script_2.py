...
from models.relate_recommenders import (
    ...
    RelateRecommenderVersion2Config
)
...
from config import (
    ...
    HBASE_PER_PROM_CSE_COLUMN, HBASE_RELATE_PROM_CSE_COLUMN,
    HBASE_USER_POSITIVE_POSTS_COLUMN, HBASE_USER_NEGATIVE_POSTS_COLUMN,
...
user_reader_config = HbaseUserReaderVersion1Config(
    ...
    positive_column=HBASE_USER_POSITIVE_POSTS_COLUMN,
    negative_column=HBASE_USER_NEGATIVE_POSTS_COLUMN,
    verbose=False
)
...
per_recommender: BasePerRecommender = PerRecommenderBuilder.build_per_recommender(
    config=PerRecommenderVersion1Config(
        ...
        per_score_reader_config=HbasePerScoreReaderVersion1Config(
            connector_config=hbase_connector_config,
            table_name=HBASE_USER_TABLE, column_name=HBASE_PER_PROM_CSE_COLUMN,
...
relate_recommender: BaseRelateRecommender = RelateRecommenderBuilder.build_relate_recommender(
    config=RelateRecommenderVersion1Config(
        ...
        relate_score_reader_config=HbaseRelateScoreReaderVersion1Config(
            connector_config=hbase_connector_config,
            table_name=HBASE_POST_TABLE, column_name=HBASE_RELATE_PROM_CSE_COLUMN,
            batch_size=30, verbose=False, stale_threshold=3*24*60*60
        ),
        backup_recommender_config=backup_recommender_config,
        user_excluded_posts_reader_config=user_excluded_posts_reader_config
...
doc_them_chan_bai_recommender: BaseRelateRecommender = RelateRecommenderBuilder.build_relate_recommender(
    config=RelateRecommenderVersion2Config(
        relate_recommender_config=RelateRecommenderVersion1Config(
            user_reader_config=None,
            relate_score_reader_config=HbaseRelateScoreReaderVersion1Config(
                connector_config=hbase_connector_config,
                table_name=HBASE_POST_TABLE, column_name=HBASE_RELATE_PROM_CSE_COLUMN,
            ...
            backup_recommender_config=None,
            user_excluded_posts_reader_config=user_excluded_posts_reader_config
        ),
        per_recommender_config=PerRecommenderVersion1Config(
            user_reader_config=None,
            per_score_reader_config=HbasePerScoreReaderVersion1Config(
                connector_config=hbase_connector_config,
                table_name=HBASE_USER_TABLE, column_name=HBASE_PER_PROM_CSE_COLUMN,
...
@app.get("/doc_them_chan_bai")
async def doc_them_chan_bai(
        user_id: int, seed_post_id: int, limit: int = 6
):
    user = User(user_id=user_id)
    seed_post = Post(post_id=seed_post_id)
    ...
    posts_scores: Optional[
        List[Tuple[Post, float]]
    ] = doc_them_chan_bai_recommender.recommend(
        user=user, seed_post=seed_post, limit=limit
    )
    # get format result
    if not posts_scores:
...
@app.get("/doc_them_chan_bai_debug")
async def doc_them_chan_bai_debug(
        user_id: int, seed_post_id: int, limit: int = 6
):
    user = User(user_id=user_id)
    seed_post = Post(post_id=seed_post_id)
    ...
    if not posts_scores:
        ...
        post_reader.read_posts(
            posts=[post for post, _ in posts_scores] + [seed_post] + user.positive_posts[:15]
        )
        return {
            "message": "Recommend successfully",
            "seed_post": {
